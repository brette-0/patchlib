...
"IPS Peek" and "Chief-NET" both allow selective patching, meaning that parts of the IPS may be excluded or included when patching by the users choice.`ips.py` is a Python ips tool, however it is not as versatile as `ipsluna` which is the *recommended*  Python IPS Module.

[ROM Patcher JS](https://www.marcrobledo.com/RomPatcher.js/), however eradicates the usage of executable "diff" appliers/makers (unless we are using Xdelta3 - which requires an x64/ARM environment that most web services cannot offer), as the tool is made entirely in JavaScript and therefore can use mod files and base files in a browser.

### How does it work?

Simple, an IPS file has a static Header reading `PATCH` and a Footer reading `EOF`. The middle of the IPS file is a series of unterminated bytearrays. These "instances" come in two forms in variable size.
...
The first element is a `24 bit` number for the  target `offset`, the second element is a `16 bit` number for the size of the data. The final element is the data of which it's length is described by the `16 bit` size number.

**Example of an RLE Instance:**

`01 23 45 00 00 FF FF 64`
If you do not know what `RLE` is then you may think that is just a smaller instance until you realize that it's `size` bytes are `00 00`.

The `16 bit` `size` number acts as an `RLE flag` when set to `0` indicating that this instance, still writing to the same offset, stores repetitive data. We read past the empty space and take the next `two bytes` as a `16 bit` number describing the `length` of the `RLE ` instance. The last byte is the `data` and will be repeated until it's length is equal to the `16 bit` size. 

Space in-between offsets beyond the size of the original file are expected to be zeroes for empty space, drastically reducing file size, likely complexity too meaning faster handling. Otherwise the space in between offsets stores the original file contents.
...
```python
def apply(base : bytes | bytearray, patch : bytes | bytearray) -> bytes | bytearray:
  ...
  output += base[len(output):]                #if we have not wrote up to base, then do so
	return output
```
The code above accepts a `bytes` or `bytearray` datatype and will return ` bytearray` datatype which could be parsed into a `file` object.  If you only needed this data for patching then you could :
```python
...
Generally, there is very minimal reason to do this. As it stands, even when `ips` filetypes are being manipulated at a deep level, the tools provided are often not even fully used as rarely does the user exceed common `building` and `applying`. There is generally a surplus of tools, should you create your own `ips` tool there should be a reason for this, `ipsluna`'s existance is to provide total control in a `Python 3.7` runtime or above.
...
- It creates some sort of proprietry sense to it, which may deter users.
